...
| `--once`                                                     | The GDB server is terminated when GDB disconnects or detaches, even when GDB runs in `extended-remote` mode. |
...
The options marked with **(+)** are ignored by dw-link.
