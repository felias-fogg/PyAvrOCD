...
The debugging system, consisting of the debug probe, the GDB server, GDB itself, and any IDE employing GDB, has a number of inherent limitations. Some aspects of the hardware may not be debuggable at all or only with some extra effort. And sometimes the behavior of the MCU in a debugging environment is significantly different from the behavior shown in a non-debugging environment. Finally, debugging your AVR chips can also be a risk to the health of your MCU.
...
If you are using PlatformIO, then you can use the project task `Disable debugWire` under the `Platform` header.
