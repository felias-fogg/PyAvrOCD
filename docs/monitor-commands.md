...
PyAvrOCD implements several `monitor` commands. These can be used to control important aspects of the GDB server. Two important commands are the `monitor debugwire enable` and `monitor debugwire disable` commands, which enable and disable, respectively, debugWIRE mode on MCUs supporting this interface. A very useful command when using a CLI interface is `monitor ioregister`, which can be used to print the values of certain I/O registers.

When using Arduino IDE 2, you can type such commands into the last line of the `Debug Console`.
...
| `monitor` `exit`                                             | First disconnect from the debug probe, and after one second, terminate. This will give the GDB client enough time to shutdown the connection first. **(+)** |
| `monitor` `help`                                             | Display help text.                                           |
| `monitor` `info`                                             | Display information about the target and the state of the debugger. |
| `monitor` `ioregister` `<ioreg-expression>` [`<integer>`]    | This monitor command displays I/O register values and can set them. An `<ioreg-expression>` is a wildcard expression (as you use for files), which can address one or more I/O registers or bitfields using the notation [`<peripheral>`.]`<register>` or  [`<peripheral>`.]`<register>`.`<field>`, respectively. If a unique register is referenced, all bitfields are also printed. If a unique bitfield is referenced, all potential values and their descriptions are printed if present in the SVD file. When an additional integer literal `<integer>` is given, then its value will be stored in the register or bitfield, provided the `<ioreg-expression>` addresses a unique register or bitfield. **(+)** |
