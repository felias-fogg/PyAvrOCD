...
- **Changed:**
     ...
     - The option `--reboot-tool` can now be given without specifying an MCU, which implies that after the reboot PyAvrOCD will immediately terminate.
     - Handling of extra arguments for simavr has changed. Now each single part has to be passed as one extra argument (see example in 'Supported Debuggers and Simulators'). For this reason, the option is now called `--xarg` and not `--xargs`.
     - All Linux PyAvrOCD binaries are now created as `onefile` and, with the help of the *staticx* Python script, are turned into static executables, avoiding (hopefully) library versioning problems. Windows and macOS are still `onedir` because Windows thinks onefile apps are a virus and macOS applies quarantine attributes to libs embedded in a zip file.
     - Binaries for ARM32 are also now generated by the CI workflow (arm6l and statically linked).
     - Calling simavr from Python has been changed to avoid using the shell (which might create problems) and to pipe the output to the debug UI.

     **Added**:

     - PyAvrOCD, simavr, and AVR-GDB binaries for i686-linux have been added.
