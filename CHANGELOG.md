...
- **Added**:
     - PyAvrOCD and AVR-GDB binaries for i686-linux has been added
- **Changed:**
     - All Linux PyAvrOCD binaries are now created as `onefile` and, with the help of the *staticx* Python script, are turned into static releases, avoiding any GLIBC versioning problems. Windows and macOS are still `onedir`, because Windows thinks it is a virus and macOS applies qurantine attributes to libs embedded in a zip file.
     - Binaries for ARM32 are also now generated by the CI workflow (arm6l and statically linked).
     - Calling simavr from Python has been changed to avoid using the shell (which might vreate problems) and to pipe the output to the debug UI.
